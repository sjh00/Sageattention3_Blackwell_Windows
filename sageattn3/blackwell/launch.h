...
void run_flash_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    // Use actual SM count (hardcoded 170 is wrong on non-B200 cards / Windows).
    auto dprops = at::cuda::getCurrentDeviceProperties();
    dim3 grid_dims = Scheduler::get_grid_dim(scheduler_args, dprops->multiProcessorCount);
    dim3 block_dims(ctaSize);
    dim3 cluster_dims(size<0>(ClusterShape{}), size<1>(ClusterShape{}), size<2>(ClusterShape{}));
    cutlass::ClusterLaunchParams launch_params{grid_dims, block_dims, cluster_dims, smem_size, stream};

#if defined(_MSC_VER)
    // MSVC cannot pass over-aligned CUTE_GRID_CONSTANT params by value.
    // Pack them into one device allocation and pass pointers instead.
