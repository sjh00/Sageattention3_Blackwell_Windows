...
// Shared device-side implementation. Host launch path differs on MSVC because
// CUTE_GRID_CONSTANT / alignas(128) kernel parameters cannot be passed by value
// (MSVC only allows up to 16-byte alignment for function parameters).
...
#if defined(_MSC_VER)
// MSVC: pass over-aligned Params by pointer (device-side pack allocated in launch.h).
// CUTE_GRID_CONSTANT + alignas(128) by-value parameters are invalid on MSVC (C2719 /
// silent misaligned TMA descriptor access at runtime).
