...
def torch_group_mean(q: torch.Tensor):
    """Per-128-token group mean centering without Triton (Windows / embed Python friendly)."""
    B, H, L, D = q.shape
    GROUP_SIZE = 128
    num_groups = L // GROUP_SIZE
    # [B, H, num_groups, GROUP_SIZE, D]
    q_grouped = q.view(B, H, num_groups, GROUP_SIZE, D)
    qm = q_grouped.mean(dim=3)  # [B, H, num_groups, D]
    q_out = (q_grouped - qm.unsqueeze(3)).view(B, H, L, D)
    return q_out.contiguous(), qm.contiguous()
